...
/** 为少量需要独立持久化的敏感前端状态创建与 Pinia 相同的加密存储。 */
export function createSecureStorage(metaKey: string): SecureLSStorage {
  return new SecureLSConstructor({
    encodingType: 'aes',
...
export async function initStores(app: App, options: InitStoreOptions) {
  ...
  const ls = createSecureStorage(`${namespace}-secure-meta`);
  pinia.use(
