...
import type { AdEventSummary, AdRuntimeConfig, SystemConfig } from '#/api';
...
const ads = ref<AdRuntimeConfig>({
  ...
  fullScreen: { enabled: true, placementId: '104516612', playbackThreshold: 5, minimumIntervalMinutes: 20, loadTimeoutMs: 8000, showTimeoutMs: 120000 },
  reward: { enabled: true, placementId: '104489019' },
  drama: {
...
async function load() {
  ...
  rows.value = configs.filter((item) => item.key !== 'ads.runtime_config');
  ads.value = adConfig;
  ...
  <Page title="参数与功能开关" description="统一维护 App 功能参数、广告位与投放频控；保存后由启动接口下发">
    <div class="flex flex-col gap-4">
      <NCard title="广告投放策略">
        ...
        <NAlert type="info" :bordered="false" class="mb-5">广告位 ID 使用 GroMore 聚合广告位。关闭总开关后，所有广告停止请求，但不会影响穿山甲短剧播放器。</NAlert>
        <NGrid cols="1 s:2 xl:4" responsive="screen" :x-gap="16" :y-gap="16">
          <NGridItem>
            <NCard size="small" title="开屏广告" embedded>
              ...
              <NFormItem label="GroMore 广告位 ID"><NInput v-model:value="ads.splash.placementId" /></NFormItem>
          ...
          <NGridItem>
            <NCard size="small" title="首页信息流" embedded>
              ...
              <NFormItem label="GroMore 广告位 ID"><NInput v-model:value="ads.feed.placementId" /></NFormItem>
              <NFormItem label="每隔多少部短剧插入"><NInputNumber v-model:value="ads.feed.insertEvery" :min="2" :max="50" class="w-full" /></NFormItem>
            </NCard>
          </NGridItem>
          <NGridItem>
            <NCard size="small" title="插全屏广告" embedded>
              ...
              <NFormItem label="GroMore 广告位 ID"><NInput v-model:value="ads.fullScreen.placementId" /></NFormItem>
          ...
          <NGridItem>
            <NCard size="small" title="激励视频" embedded>
              ...
              <NFormItem label="GroMore 广告位 ID"><NInput v-model:value="ads.reward.placementId" /></NFormItem>
      ...
      <NCard title="短剧 SDK 播放与解锁">
        <NAlert type="warning" :bordered="false" class="mb-5">
          “平台安全模式”使用 SDK 内置广告，只负责解锁，不进入幻乐金币结算；“自有结算模式”使用 GroMore 激励广告、服务端回调和后台分成开关。App ID、安全密钥及 SDK 内置广告位来自安装包内签名配置，不能热更新。
        </NAlert>
        <NGrid cols="1 m:2 xl:3" responsive="screen" :x-gap="16" :y-gap="4">
          <NGridItem><NFormItem label="解锁广告模式"><NSelect v-model:value="ads.drama.unlockMode" :options="[{label:'自有结算模式（推荐）',value:'SPECIFIC'},{label:'SDK 平台安全模式',value:'COMMON'}]" /></NFormItem></NGridItem>
          <NGridItem><NFormItem label="免费集数"><NInputNumber v-model:value="ads.drama.freeEpisodes" :min="0" :max="1000" :precision="0" class="w-full" /></NFormItem></NGridItem>
          <NGridItem><NFormItem label="每次解锁集数"><NInputNumber v-model:value="ads.drama.unlockEpisodes" :min="1" :max="10" :precision="0" class="w-full" /></NFormItem></NGridItem>
        </NGrid>
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <div class="flex items-center justify-between rounded-lg border border-gray-700 px-4 py-3"><span>连续解锁</span><NSwitch v-model:value="ads.drama.continuousUnlock" /></div>
          <div class="flex items-center justify-between rounded-lg border border-gray-700 px-4 py-3"><span>隐藏 SDK 奖励弹窗</span><NSwitch v-model:value="ads.drama.hideRewardDialog" /></div>
          <div class="flex items-center justify-between rounded-lg border border-gray-700 px-4 py-3"><span>隐藏蜂窝网络提示</span><NSwitch v-model:value="ads.drama.hideCellularToast" /></div>
          <div class="flex items-center justify-between rounded-lg border border-gray-700 px-4 py-3"><span>允许无限滑动</span><NSwitch v-model:value="ads.drama.infiniteScrollEnabled" /></div>
          <div class="flex items-center justify-between rounded-lg border border-gray-700 px-4 py-3"><span>隐藏点赞按钮</span><NSwitch v-model:value="ads.drama.hideLikeButton" /></div>
          <div class="flex items-center justify-between rounded-lg border border-gray-700 px-4 py-3"><span>隐藏收藏按钮</span><NSwitch v-model:value="ads.drama.hideFavorButton" /></div>
          <div class="flex items-center justify-between rounded-lg border border-gray-700 px-4 py-3"><span>关闭双击点赞</span><NSwitch v-model:value="ads.drama.hideDoubleClick" /></div>
          <div class="flex items-center justify-between rounded-lg border border-gray-700 px-4 py-3"><span>关闭长按倍速</span><NSwitch v-model:value="ads.drama.hideLongClickSpeed" /></div>
        </div>
        <div class="mt-5 flex justify-end"><NButton v-if="canUpdate" type="primary" :loading="savingAds" @click="saveAds">保存短剧 SDK 配置</NButton></div>
      </NCard>
