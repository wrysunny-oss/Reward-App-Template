...
const baseOpenApiDocument = {
  ...
  paths: {
    "/auth/register/sms-code": { post: { tags: ["认证"], summary: "发送注册短信验证码", description: "向未注册的中国大陆手机号发送 6 位验证码；同一手机号存在重发冷却，接口另受 IP 限流保护。", requestBody: jsonBody({ phone: { type: "string", example: "13800138000", pattern: "^1\\d{10}$" } }, ["phone"]), responses: ok("返回重发冷却秒数和验证码有效期") } },
    "/auth/register": { post: { tags: ["认证"], summary: "用户注册", description: "校验短信验证码和邀请码后创建用户、绑定邀请关系并发放注册奖励。", requestBody: jsonBody({ phone: { type: "string", example: "13800138000" }, smsCode: { type: "string", pattern: "^\\d{6}$", example: "123456" }, password: { type: "string", minLength: 8 }, nickname: { type: "string" }, inviteCode: { type: "string", pattern: "^[A-Z0-9]{6,12}$" } }, ["phone", "smsCode", "password", "nickname", "inviteCode"]), responses: { ...ok("注册成功"), "201": { description: "注册成功并返回访问令牌" }, "404": { description: "邀请码不存在" } } } },
    ...
    "/content/categories": { get: { tags: ["短剧"], summary: "短剧实时分类", description: "实时获取穿山甲短剧二级分类，返回分类 ID、名称和上级名称。", responses: ok() } },
    "/content/dramas": { get: { tags: ["短剧"], summary: "短剧列表", description: "配置内容 Server Key 后实时返回穿山甲物料并标记 provider=PANGLE；未配置时返回本地已发布内容。", parameters: [...pageParameters, { in: "query", name: "category", description: "本地短剧分类", schema: { type: "string" } }, { in: "query", name: "categoryId", description: "穿山甲二级分类 ID", schema: { type: "integer", minimum: 1 } }, { in: "query", name: "keyword", description: "标题关键词", schema: { type: "string" } }], responses: ok() } },
    "/content/dramas/{id}": { get: { tags: ["短剧"], summary: "短剧详情", description: "获取已发布短剧及其可播放剧集。", parameters: idParameter("id", "短剧 ID"), responses: ok() } },
    "/content/playback-validations": { post: { tags: ["短剧"], summary: "上报真机播放验证", description: "接收 Pangrowth 原生 SDK 的内容请求成功与真实开播回执；只有开播回执会将内容标记为真机验证通过。", security: bearer, requestBody: jsonBody({ externalId: { type: "string" }, eventType: { type: "string", enum: ["REQUEST_SUCCEEDED", "PLAYBACK_STARTED"] }, episodeIndex: { type: "integer", minimum: 1 }, deviceModel: { type: "string" }, abi: { type: "string" }, sdkVersion: { type: "string" } }, ["externalId", "eventType", "episodeIndex"]), responses: ok() } },
    ...
    "/rewards/center": { get: { tags: ["奖励中心"], summary: "奖励中心概览", description: "返回个人邀请码、直推/间推邀请奖励配置、签到数据，以及服务端统计的今日激励广告完成次数和后台配置上限。", security: bearer, responses: ok() } },
    "/rewards/ad-rewards/verified-impression": { post: { tags: ["奖励中心"], summary: "结算已验真的非激励广告曝光", description: "接收开屏、信息流或全屏广告的加密 rs_info；后端用 rss-key 核对代码位、eCPM 和请求 ID 后按后台比例幂等发币。", security: bearer, requestBody: jsonBody({ format: { type: "string", enum: ["SPLASH", "FEED", "FULL_SCREEN"] }, placementId: { type: "string" }, slotId: { type: "string" }, requestId: { type: "string" }, ecpm: { type: "string" }, rsInfo: { type: "string" }, adnName: { type: "string" } }, ["format", "placementId", "slotId", "requestId", "ecpm", "rsInfo"]), responses: { ...ok(), "201": { description: "结算成功" }, "422": { description: "加密凭证校验失败" }, "503": { description: "服务端尚未配置 rss-key" } } } },
    ...
    "/rewards/ad-rewards/{transactionId}/status": { get: { tags: ["奖励中心"], summary: "查询广告奖励到账状态", description: "使用 SDK 返回的交易号查询当前用户对应的服务端回调与金币结算结果。", security: bearer, parameters: [{ in: "path", name: "transactionId", required: true, schema: { type: "string", minLength: 1, maxLength: 100 } }], responses: ok() } },
    ...
    "/rewards/invite/relations": { get: { tags: ["奖励中心"], summary: "我的邀请关系", description: "只返回当前登录用户的直属上级、直推与间推分页列表。账号使用公开 UID，并按业务要求返回关系成员手机号；不允许指定其他目标用户。", security: bearer, parameters: pageParameters, responses: ok() } },
    ...
    "/rewards/check-ins": { post: { tags: ["奖励中心"], summary: "每日签到", description: "按北京时间自然日幂等签到；重复提交返回首次签到结果，不会重复发放金币。", security: bearer, responses: { ...ok(), "201": { description: "签到成功或返回今日已有结果" } } } },

    "/withdrawals/config": { get: { tags: ["提现"], summary: "提现规则", description: "返回开关、金币兑换比例、单次与每日限额及手续费。", security: bearer, responses: ok() } },
    "/withdrawals": { post: { tags: ["提现"], summary: "申请提现", description: "使用 requestId 幂等创建订单，将可用金币转为冻结金币；收款渠道、实名和账号从用户预先绑定的收款账户读取，客户端不得在提现请求中重复提交。", security: bearer, requestBody: jsonBody({ requestId: { type: "string", minLength: 8, maxLength: 64 }, coins: { type: "string", description: "提现金币正整数" } }, ["requestId", "coins"]), responses: { ...ok(), "201": { description: "申请成功" }, "409": { description: "余额不足或提现关闭" }, "422": { description: "未绑定收款账户或金币不符合规则" }, "429": { description: "超过每日限额" } } } },
    ...
    "/operations/documents/{code}": { get: { tags: ["运营配置"], summary: "最新发布文档", description: "按稳定编码获取用户协议、隐私政策或关于我们的最新发布版本。", parameters: [{ in: "path", name: "code", required: true, schema: { type: "string", example: "USER_AGREEMENT" } }], responses: ok() } },
    ...
    "/notifications": { get: { tags: ["通知"], summary: "我的站内信", description: "返回最近 100 条奖励、提现、系统及活动通知和未读数量。", security: bearer, responses: ok() } },
    "/notifications/unread-count": { get: { tags: ["通知"], summary: "未读通知数", description: "用于首页消息红点。", security: bearer, responses: ok() } },
    "/notifications/{id}/read": { put: { tags: ["通知"], summary: "标记单条已读", security: bearer, parameters: idParameter("id", "通知 ID"), responses: ok() } },
    "/notifications/read-all": { put: { tags: ["通知"], summary: "全部标记已读", security: bearer, responses: ok() } },
    ...
    "/safety/device-risk-challenges": { post: { tags:["反馈风控"],summary:"创建设备检测挑战",description:"创建两分钟内有效且只能使用一次的随机挑战。提交检测时 evidence.challengeNonce 必须与挑战一致。",security:bearer,requestBody:jsonBody({context:{type:"string",enum:["login","reward","withdrawal"]}},["context"]),responses:{...ok(),"201":{description:"挑战创建成功"}} } },
    "/safety/device-presence": { post:{tags:["反馈风控"],summary:"更新前台设备活跃位置",description:"APP 在前台时每五分钟上报一次。只维护附近设备聚集统计的短期位置，不直接评分或处罚；模拟、过旧和低精度位置不会入池。",security:bearer,requestBody:jsonBody({deviceId:{type:"string"},location:{type:"object",properties:{latitude:{type:"number"},longitude:{type:"number"},accuracyMeters:{type:"number"},capturedAt:{type:"integer"},isMock:{type:"boolean"}}}},["deviceId","location"]),responses:ok()} },
    "/safety/device-risk-assessments": { post: { tags:["反馈风控"],summary:"提交设备环境检测",description:"提交一次性挑战和 Android 原生采集结果。服务端按定位精度、新鲜度和模拟定位标记维护 APP 活跃设备位置，并统计配置半径内的唯一设备数；UNKNOWN 不扣分。生产环境满足有效项数量且低于阈值时自动封号。",security:bearer,requestBody:jsonBody({challengeId:{type:"string",format:"uuid"},deviceId:{type:"string"},simStatus:{type:"string",enum:["PASS","RISK","UNKNOWN"]},wechatStatus:{type:"string",enum:["PASS","RISK","UNKNOWN"]},douyinStatus:{type:"string",enum:["PASS","RISK","UNKNOWN"]},alipayStatus:{type:"string",enum:["PASS","RISK","UNKNOWN"]},emulatorStatus:{type:"string",enum:["PASS","RISK","UNKNOWN"]},cloudDeviceStatus:{type:"string",enum:["PASS","RISK","UNKNOWN"]},scriptStatus:{type:"string",enum:["PASS","RISK","UNKNOWN"]},networkStatus:{type:"string",enum:["PASS","RISK","UNKNOWN"]},ipStatus:{type:"string",enum:["PASS","RISK","UNKNOWN"]},location:{type:"object",properties:{latitude:{type:"number"},longitude:{type:"number"},accuracyMeters:{type:"number"},capturedAt:{type:"integer"},isMock:{type:"boolean"}}},evidence:{type:"object",properties:{challengeNonce:{type:"string"}}}},["challengeId","deviceId","simStatus","wechatStatus","douyinStatus","alipayStatus","emulatorStatus","cloudDeviceStatus","scriptStatus","networkStatus","ipStatus","evidence"]),responses:{...ok(),"201":{description:"评分完成"}} } },
    ...
    "/safety/reports/mine": { get: { tags:["反馈风控"],summary:"我的举报",description:"查看当前用户提交的举报、平台判定与处理轨迹。",security:bearer,responses:ok() } },
    ...
    "/admin/dashboard": { get: { tags: ["后台-用户"], summary: "运营看板", description: "返回用户、短剧、反馈、金币、近七日用户及广告趋势、业务状态构成，以及待处理对账异常和最近对账状态。需要 dashboard:read。", security: bearer, responses: ok() } },
    "/admin/users": {
    ...
    "/admin/withdrawals/{id}/complete": { post: { tags: ["后台-用户"], summary: "确认打款结果", description: "PAYING 只能进入 COMPLETED 或 FAILED；确认成功必须填写支付流水号，失败时自动退回冻结金币。需要 withdrawal:review。", security: bearer, parameters: [{ in: "path", name: "id", required: true, schema: { type: "string", format: "uuid" } }], requestBody: jsonBody({ success: { type: "boolean" }, remark: { type: "string", minLength: 2 }, paymentReference: { type: "string", description: "success=true 时必填" } }, ["success", "remark"]), responses: ok() } },
    "/admin/withdrawal-config": {
      ...
      put: { tags: ["后台-用户"], summary: "更新提现规则", description: "更新固定提现档位、兑换比例、每日限额、手续费和开关并写入审计。APP 只能提交后台开放的档位。需要 withdrawal:config。", security: bearer, requestBody: jsonBody({ enabled: { type: "boolean" }, coinsPerCent: { type: "integer" }, tiers: {type: "array", items: {type: "string"}, minItems: 1, maxItems: 20}, dailyCountLimit: { type: "integer" }, dailyCoinLimit: { type: "string" }, feeRateBps: { type: "integer", minimum: 0, maximum: 10000 } }, ["enabled", "coinsPerCent", "tiers", "dailyCountLimit", "dailyCoinLimit", "feeRateBps"]), responses: ok() },
    },
    ...
    "/admin/withdrawal-batches/{id}/alipay-pay": { post: { tags: ["资金对账"], summary: "启动支付宝批量打款", description: "异步逐笔处理批次内待打款的支付宝订单；使用固定商户单号保证重试幂等。需要批次管理权限和二次密码。", security: bearer, parameters: [...idParameter("id", "批次 ID"), { in: "header", name: "X-Confirm-Password", required: true, schema: { type: "string", format: "password" } }], responses: { ...ok(), "202": { description: "批量任务已启动" } } } },
    "/admin/withdrawal-batches/{id}/alipay-query": { post: { tags: ["资金对账"], summary: "启动支付宝批量查单", description: "异步查询批次内未确认订单，并按支付宝权威结果完成结算或退款。需要批次管理权限和二次密码。", security: bearer, parameters: [...idParameter("id", "批次 ID"), { in: "header", name: "X-Confirm-Password", required: true, schema: { type: "string", format: "password" } }], responses: { ...ok(), "202": { description: "批量查单任务已启动" } } } },
    ...
    "/admin/announcements": {
      ...
      post: { tags: ["后台-运营"], summary: "创建公告", description: "创建公告内容、发布状态和展示有效期。需要 operation:update。", security: bearer, requestBody: jsonBody({ title: { type: "string" }, content: { type: "string" }, status: { type: "string", enum: ["DRAFT", "PUBLISHED", "OFFLINE"] }, startAt: {type: "string", format: "date-time", nullable: true}, endAt: {type: "string", format: "date-time", nullable: true} }, ["title", "content", "status"]), responses: { ...ok(), "201": { description: "创建成功" } } },
    },
    ...
    "/admin/notifications": { post: { tags: ["后台-运营"], summary: "发送 App 站内信", description: "向全部活跃用户或指定手机号发送系统、活动、奖励或提现通知，并记录审计日志。需要 operation:update 和二次密码。", security: bearer, requestBody: jsonBody({ target: { type: "string", enum: ["ALL", "USER"] }, phone: { type: "string", description: "target=USER 时必填" }, type: { type: "string", enum: ["SYSTEM", "PROMOTION", "REWARD", "WITHDRAWAL"] }, title: { type: "string", maxLength: 150 }, content: { type: "string", maxLength: 5000 } }, ["target", "type", "title", "content"]), responses: { ...ok(), "201": { description: "发送成功" } } } },
    ...
    "/admin/app-versions": {
      ...
      post: { tags: ["后台-运营"], summary: "创建 App 版本", description: "创建更新说明、下载地址、最低版本、灰度比例和发布时间。需要 operation:update。", security: bearer, requestBody: jsonBody({ platform: { type: "string", enum: ["ANDROID", "IOS"] }, versionName: { type: "string" }, versionCode: { type: "integer" }, minVersionCode: { type: "integer" }, downloadUrl: { type: "string", format: "uri" }, releaseNotes: { type: "string" }, enabled: { type: "boolean" }, rolloutPercent: { type: "integer", minimum: 0, maximum: 100 }, publishedAt: {type: "string", format: "date-time", nullable: true} }, ["platform", "versionName", "versionCode", "minVersionCode", "downloadUrl", "releaseNotes", "enabled", "rolloutPercent"]), responses: { ...ok(), "201": { description: "创建成功" } } },
    },
    ...
    "/admin/content-center": { get: { tags: ["后台-运营"], summary: "短剧内容中心", description: "分页查看短剧快照、第三方内容 ID、集数和播放映射状态。需要 operation:read。", security: bearer, parameters: pageParameters, responses: ok() } },
    "/admin/content-center/sync": { post: { tags: ["后台-运营"], summary: "同步最新短剧", description: "从穿山甲同步最新一页短剧快照并写入审计日志。需要 operation:update。", security: bearer, responses: ok() } },
    "/admin/sdk-health": { get: { tags: ["后台-运营"], summary: "SDK 与内容服务健康状态", description: "返回脱敏后的内容配置、广告回调和客户端验证边界。需要 operation:read。", security: bearer, responses: ok() } },
    "/admin/sdk-health/content-probe": { post: { tags: ["后台-运营"], summary: "探测穿山甲内容接口", description: "实时调用分类接口并返回耗时和结果，不返回 Server Key。需要 operation:read。", security: bearer, responses: ok() } },
    ...
    "/admin/risk-rules": { get:{tags:["反馈风控"],summary:"风控规则目录",description:"返回十项设备环境检测的通过条件、风险条件、每项扣分、数据来源、未知项处理方式及当前自动封号阈值。需要 risk:read。",security:bearer,responses:ok()} },
    "/admin/risk-policy": {
      get:{tags:["反馈风控"],summary:"读取风控策略",description:"读取自动风控开关、评分阈值、附近设备聚集参数和关联账号阈值。需要 risk:read。",security:bearer,responses:ok()},
      put:{tags:["反馈风控"],summary:"更新风控策略",description:"策略立即生效并记录审计日志。可配置附近设备半径、设备数、活跃窗口、定位精度和位置保留时间。需要 risk:update 和二次密码。",security:bearer,requestBody:jsonBody({enabled:{type:"boolean"},requireFreshAssessment:{type:"boolean"},minKnownChecks:{type:"integer",minimum:1,maximum:10},autoBanThreshold:{type:"integer"},warningThreshold:{type:"integer"},nearbyRadiusMeters:{type:"integer"},nearbyDeviceThreshold:{type:"integer"},presenceWindowMinutes:{type:"integer"},maxLocationAccuracyMeters:{type:"integer"},maxLocationAgeSeconds:{type:"integer"},locationRetentionHours:{type:"integer"},loginValidityHours:{type:"integer"},rewardValidityMinutes:{type:"integer"},withdrawalValidityMinutes:{type:"integer"},multiAccountDeviceThreshold:{type:"integer"},multiAccountIpThreshold:{type:"integer"}},["enabled","requireFreshAssessment","minKnownChecks","autoBanThreshold","warningThreshold","nearbyRadiusMeters","nearbyDeviceThreshold","presenceWindowMinutes","maxLocationAccuracyMeters","maxLocationAgeSeconds","locationRetentionHours","loginValidityHours","rewardValidityMinutes","withdrawalValidityMinutes","multiAccountDeviceThreshold","multiAccountIpThreshold"]),responses:ok()},
    },
    ...
    "/admin/alipay-payout/status": { get: { tags: ["资金管理"], summary: "支付宝打款配置状态", description: "仅返回配置是否完整、接入模式和缺失项，不返回密钥或证书内容。需要 withdrawal:read。", security: bearer, responses: ok() } },
    "/admin/withdrawals/{id}/alipay-transfer": { post: { tags: ["资金管理"], summary: "发起支付宝单笔打款", description: "仅允许对已审核且处于 PAYING 状态的支付宝提现单发起转账；提现 ID 固定映射为支付宝商户单号以保证幂等。需要 withdrawal:review 和二次密码验证。", security: bearer, parameters: idParameter("id", "提现 ID"), responses: ok() } },
    "/admin/withdrawals/{id}/alipay-query": { post: { tags: ["资金管理"], summary: "查询支付宝打款结果", description: "查询支付宝权威结果并同步提现状态；明确成功后结算冻结金币，明确失败后退款，处理中保持 PAYING。需要 withdrawal:review 和二次密码验证。", security: bearer, parameters: idParameter("id", "提现 ID"), responses: ok() } },
